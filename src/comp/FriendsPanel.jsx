    ...
    <div className="bg-primaryBackground max-w-sm py-4 pr-4 pl-4">
      ...
      <div className="px-4">
        ...
        <div className="space-x-1 pl-1 pt-1">
          ...
          <a className="text-xs no-underline font-medium cursor-pointer" onClick={() => setSearch('')}>Clear</a>
        </div>
      </div>

      <FriendUsersList users={userSearchResults.slice(activeIndex * 3, activeIndex * 3 + 3)} {...rest} />
      <div className="flex justify-end px-4 space-x-5 text-xs font-bold">
        {[0,1,2].map((idx) => <a onClick={() => setActiveIndex(idx)} className={`${activeIndex === idx ? 'text-black' : ''} no-underline`}>{idx}</a>)}
      </div>
